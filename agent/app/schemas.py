...
NOTE (Kaggle deploy / self-hosted small models): the coercion validators below
(marked "weak-model coercion") were added because self-hosted models such as
gemma-3n-E4B-it / gemma-4-E4B-it do not reliably follow the nested JSON shapes
this schema expects — the original design targeted GPT-4o-mini, which was much
more consistent. Each coercion only *normalizes* obviously-equivalent shapes
(e.g. a bare list where an object was expected); it never invents data.
"""
...
class CaseDraft(BaseModel):
    ...
    @classmethod
    def _coerce_prediction(cls, v: Any) -> Any:
        """Some models return the bare label string ("PARTIAL_A_WIN") instead
        of the nested object {"prediction": "PARTIAL_A_WIN"} AlqacPrediction
        expects. Wrap it if so."""
        if isinstance(v, str):
            return {"prediction": v}
        return v
...
class EntityNode(BaseModel):
    ...
    def _coerce_attributes(cls, v: Any) -> dict[str, Any]:
        """Some models return [] instead of {} when an entity has no
        attributes. Normalize any list shape into a dict."""
        if v is None:
            return {}
...
class ElementGraph(BaseModel):
    """Nested entity/event/relationship graph extracted from the case text.

    Weak/local models sometimes flatten the whole output into a bare list of
    entities instead of the nested {"entities": [...], ...} shape this model
    ...
    @classmethod
    def _coerce_bare_entity_list(cls, data: Any) -> Any:
...
class ManagerDecision(BaseModel):
    ...
    @model_validator(mode="before")
    @classmethod
    def _coerce_decision_action_confusion(cls, data: Any) -> Any:
